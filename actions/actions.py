...
intent_responses={}
def init_config():
    global pseb_API_url, conversation_file_path, intent_responses
    api_config = json.load(open("C:\\Users\\ahsan\\Downloads\\FinalPSEBChatbotFolder\\three_Check\\actions\\Data_Config.json"))

    pseb_API_url = api_config['pseb_API_url']
    conversation_file_path = api_config['conversation_file_path']
    faq_intent_path = api_config['faq_intent_path']
    faq_intent_file = api_config['faq_intent_file']
    intent_responses = pd.read_csv(faq_intent_path+faq_intent_file)
...
class ActionDefaultFallback(Action):
    ...
    def run(
        ...
        msg=intent_responses.loc[intent_responses.intent == intent]['response'][0]
